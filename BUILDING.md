...
把 `human-task` 打包成**可分发的预构建产物**（三个 `.tgz`，落到 `build/`）。全新用户**不需要**做这些——他们（或安装 agent）只消费产物，见 [INSTALL.md](INSTALL.md)。
...
以上三点**已实测确认**。所以完整构建（生成 `/remote` 面与客户端 bundle）**只能发生在 harness 源码工作区**——那里 `dsh-typert-protocol` 本身就是 `packages/` 成员。

本仓库的构建脚本 `scripts/harness-build.mjs` 会自动 checkout/定位 harness 源码、把三个包复制进去、改写成工作区约定并构建打包。
...
`pnpm pack` 会把 `workspace:^` 重写为具体版本（`^0.1.0-rc.x`），所以 `.tgz` 里的 peer 依赖是可被运行时解析的 semver 范围。
...
见 [build/README.md](build/README.md) 与 [INSTALL.md](INSTALL.md)：三个 `.tgz` 装进 `$DSH_HOME/profiles/<name>/node_modules/@deepseek-ai/` 下（或 profile `package.json` 依赖），`install/cordis.patch.yml` 并入用户 patch 层，`preset/human-task/` 复制到 `$DSH_HOME/.agent-presets/`。
